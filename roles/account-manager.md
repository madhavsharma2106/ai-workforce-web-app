...
- Answers to ad hoc questions from Emma and Oliver (e.g. "is this company a good fit?", "which case study fits this email?") via the `ask_account_manager` tool — a nested, one-shot turn as Alex grounded in the Business Profile (and the specific lead, if the question is about one), not a `delegate_to_employee` handoff

## Integrations

- The `business_profiles` table (see [docs/DATABASE.md](../docs/DATABASE.md)) is wired: the onboarding conversation at `/employee/:id/onboarding` (for the auto-hired `account_manager` employee) writes it via `POST /api/employees/[id]/complete-onboarding`, and every employee's system prompt includes `profile_md` wholesale on every run — see [src/lib/employees.ts](../src/lib/employees.ts) and [src/lib/agents/systemPrompt.ts](../src/lib/agents/systemPrompt.ts) (`buildSystemPrompt`). On top of that passive channel, Emma and Oliver can call `ask_account_manager` (`src/lib/agents/tools/askAccountManagerTool.ts`) mid-run for a live answer to a specific question, grounded in the same profile. Design: the Business Profile is stored as markdown (a `profile_md` column) — the same form other roles' docs take — since it's read whole as LLM prompt context rather than queried field-by-field; a few structured columns (`user_id`, `updated_at`, `business_name`, `contact_name`) support the app-level lookups that don't need the full document. The onboarding question set covers all five Business Profile components (business description, ideal client, bad-fit criteria, value proposition, tone), plus optional current priorities and do's/don'ts — refined further through ongoing check-ins rather than a longer upfront form.
- Once onboarding is done, "Talk to Alex" (`KnowledgeRefreshModal`, on Alex's own page) reopens that conversation for updates rather than requiring a form edit: `POST /api/employees/[id]/knowledge-gaps` asks only about what's genuinely missing from the current profile, and `POST /api/employees/[id]/apply-knowledge-refresh` merges the answers into `profile_md` on completion via `mergeBusinessProfile` — see [docs/DATABASE.md](../docs/DATABASE.md). The same modal, on Emma's or Oliver's own page, refreshes their `instructions_md` instead.
