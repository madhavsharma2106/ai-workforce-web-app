...
| `employees` | One row per hired employee (see [UX.md](UX.md): Role vs. Employee) | `id` (PK), `user_id` (→ `auth.users`), `role` (`account_manager` \| `lead_sourcer`; `sales_representative` is planned for the Sales Representative role — see [roles/sales-representative.md](../roles/sales-representative.md) — but not yet in the check constraint, code `EmployeeRole` type, or `delegations.to_role`), `status` (`onboarding` \| `active`), `created_at`. A partial unique index enforces at most one `account_manager` row per user. |
...
`business_profiles` now has real writers/readers: Account Manager onboarding and her `/employee/:id` dashboard's "Update profile" flow (`/api/employees/[id]/complete-onboarding` and `/api/employees/[id]/business-profile`) both write it via the shared `buildProfileMarkdown`/`parseProfileMarkdown` helpers in [src/lib/businessProfile.ts](../src/lib/businessProfile.ts); Lead Sourcer's first run reads `profile_md` as the search input — see [src/lib/employees.ts](../src/lib/employees.ts) and [src/lib/leadSearch.ts](../src/lib/leadSearch.ts).
