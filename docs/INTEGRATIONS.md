...
|---|---|---|---|---|
| Apollo.io | `src/lib/integrations/apollo.ts` | Lead search (`searchPeople`) and email reveal (`revealEmail`) | `APOLLO_API_KEY` | Lead Sourcer, via `src/lib/agents/graph.ts`'s pre-fetch phase and `src/app/api/leads/reveal-email` |
| Company websites | `src/lib/integrations/companyWebsite.ts` | Fetches and cleans a company's homepage text for the model to reason over (`fetchCompanyText`) — not a distinct third-party product, just plain `fetch` against whatever domain Apollo returned | none | Lead Sourcer, via `src/lib/agents/graph.ts`'s pre-fetch phase |
